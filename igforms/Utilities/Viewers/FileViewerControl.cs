...
namespace IG.Forms
{
    public partial class FileViewerControl : UserControl, IFileViewer
    {
        ...
        protected static long _lastWebStreamLength = 0;
        ...
        public static string GetTextFromUrl(string url)
        {
            ...
            using (HttpWebResponse httpWebReponse = (HttpWebResponse)httpWebRequest.GetResponse())
            {
                using (Stream stream = httpWebReponse.GetResponseStream())
                {
                    ...
                    {
                        _lastWebStreamLength = stream.Length;
        ...
        public void ShowViewedItem()
        {
            ...
            if (!string.IsNullOrEmpty(ViewedFile))
            {
                ...
                {
                    txtOutput.Text = File.ReadAllText(ViewedFile);
                } else if (CanViewUrls && IsValidHttpUrl(_viewedFile))
                {
                    txtOutput.Text = GetTextFromUrl(ViewedFile);
                }
                else
                {
            ...
            {
                StreamReader reader = new StreamReader(ViewedStream);
                {
                    long position = ViewedStream.Position;
                    ViewedStream.Position = 0;
                    txtOutput.Text = reader.ReadToEnd();
                    ViewedStream.Position = position;  // reatore stream position at the end
                }
            }
        }
        ...
        public bool IsEligibleForViewing(string fileLocation)
        {
            ...
            if (CanViewUrls && IsValidHttpUrl(fileLocation))
            {
                try
                {
                    string str = GetTextFromUrl(fileLocation);
                    ret = true;
                }
                catch { }
                if (ret)
